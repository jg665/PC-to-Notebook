...
1. 개요
	1) 데이터베이스에 저장되는 데이터는 문자, 숫자, 날짜, 이미지 등과 같은
		댜양한 형식으로 표현된다.
	2) 실무에서 컬럼에 대한 데이터 타입이나 길이의 결정은 데이터베이스 설계 단계에서
		신중하게 고려해야 하는 중요한 사항
	3) 데이터베이스 설계 단계에서 잘못 선정된 데이터 타입은 응용 프로그램 개발을
...
2. 오라클에서 지원되는 데이터 타입의 종류
	...
	4) date : 날짜/시간 형식을 저장하기 위한 데이터 타입
		SYSDATE : 현재 오라클 시스템 날짜를 저장하거나 불러올 때, 사용된다.
*/
...
/*		
	...
	9) bfile : 대용량의 바이너리 데이터를 파일 형태로 저장하ㅣㄱ 위한 데이터 타입 최대 4gb
...
--							테이블의 구조의 순서대로 입력
INSERT INTO MEMBER values('himan','7777','9111112111111',1000,'관리자');
SELECT * FROM MEMBER;

-- ex) 프로양구 팀 정보를 입력하려고 한다. 구글에 프로야구 순위표 확인하여
