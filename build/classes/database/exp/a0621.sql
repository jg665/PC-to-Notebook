...
/*9. decode와 case 함수의 기본형식을 기술하세요.
	...
	짝수의 케이스를 지정해서 해당 데이터일떄, switch case와 비슷한 구조로
	되어 있다.
	case 컬럼
...
-- ex1) 분기별 최고 급여자가 소속된 부서정보를 출력하세요.
-- 1) 분기별 최고 급여자 
...
-- 3) 분기별 최고 급여자 부서정보와 join된 사원테이블을 조인관게를
--	  설정하여, 부서정보를 출력하여야 한다.
	SELECT d.*
	FROM (
	-- 분기별 최고 급여자 ==> 부서정보를 출력하세요.
	-- 분기별 최고 급여자 ==> 사원정보 ==> 부서정보.
	-- 1) 분기별 최고 급여자 ==> 사원정보 : 부서정보, 급여정보를 조인
	-- 2) 사원정보와 부서정보 : 부서번호로 조인
		SELECT to_char(hiredate, 'Q') q, max(sal) msal
		FROM emp 
		GROUP BY to_char(hiredate, 'Q')
		HAVING max(SAL)>=2000) a, emp e, dept d
	WHERE a.q = to_char(e.hiredate, 'Q') -- 사원정보에서 분기정보를 조인
...
(SELECT * FROM dept WHERE detpno = 10) d
WHERE e.deptno = d.deptno;
--	3) select subquery
SELECT (SELECT max(sal) 
		FROM emp 
		WHERE deptno=a.depetno) "소속부서최고급여", 
	a.*
FROM emp a;
