...
1. 일반 조인과 outer join의 차이점을 기술하세요.
	...
	join
	1). 하나의 SQL명령문에 의해 여러 테이블에 저장된 데이터를 한번에 조회할 수 있다.
...
3. self 조인이 무엇인지 emp 테이블의 empno, mgr을 기준으로 설명하세요./*
	같은 테이블내부에서 존재하는 컬럼과 컬럼끼리 조인관계를 설정하는 것을 말한다.
...
2. 다중결과 데이터 처리 : in, exists, any, all
	결과값이 하나이상일 때, 해당 다중의 결과를 기준으로 데이터를 처리할 때 활용된다.
...
/* 분기별 최고급여가 같은 경우...1분기, 4분기가 같은 경우
	위와 같은 경우에 1분기 뿐만 아니라 4분기 데이터도 같이 로딩이 된다.
