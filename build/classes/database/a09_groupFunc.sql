...
/*
# 그룹함수란?
1. 테이블의 전체 행을 하나 이상의 컬름을 기준으로 그룹화하여 
	그룹별로 결과를 출력하는 함수
...
-- ex2) 2사분기입사한 사람의 job의 갯수를 출력하세요 [6조]
SELECT job
FROM emp 
...
SELECT *
FROM emp;
-- ex1) 연봉이 2000~5000사이에 연봉의 합산, 최대, 평균값을 출력하세요
SELECT sum(sal), max(sal), avg(sal)
FROM emp 
WHERE sal BETWEEN 2000 AND 5000;

-- ex2) 2사분기에 입사한 사람의 연봉 합산, 최대, 최소, 평균값을 출력하세요.[5조]
SELECT sum(sal), max(sal), min(sal), avg(sal)
FROM emp 
WHERE to_char(hiredate, 'Q') = '2';
/*
# group by절
1. 특정 칼럼 값을 기준으로 테이블의 전체 행을 그룹별로 나누기 위한 절
...
3. SELECT 절에서 나열된 칼럼 이름이나 표현식은 GROUP BY 절에서 반드시 명시
4. GROUP BY 절에서 명시한 컬럼 이름은 SELECT절에서 명시하지 않아도 된다.
 **/
-- 부서별평균급여
...
GROUP BY deptno;
SELECT * FROM emp;
-- ex1) 직책(job)별 평균급여와 최대 급여를 출력하세요
SELECT job "직책", avg(sal) "평균급여", max(sal) "최대급여"
FROM emp
GROUP BY job;
-- ex2) 분기별 최대 급여와 최저 급여를 출력하세요 [4]
...
--ex2) 직책별 인원이 3명이상인 직책과 인원을 출력하세요 [3]
SELECT * FROM emp;
