...
1. 특정한 문자열을 검색해서 해당 문자열의 위치를 index로
	...
	ex) 주로 대용량의 문자열 데이터를 검색해서 해당 문자의
		...
		필요로 한다.
2. instr(지정한문자열데이터|컬럼, '검색문자',검색된횟수)
*/
...
-- 해당 문자가 없을 때는 0, 있을 때는 순번 index(1부터 시작)
-- 처리해준다.
SELECT * FROM emp;
/*
...
SELECT 
round(243323.235,3), -- 소숫점. 기준 오른쪽3번째
round(243323.233,1), -- 소숫점. 기준 오른쪽 1번째
round(243323.235,-1), -- 소숫점. 기준 왼쪽 1번째
round(243325.233,-2),
round(243353.233,-3)
FROM dual;
-- ex ) 사원명과 sal을 출력하되, sal 15% 인상금액을 100자리
--		단위와 소숫점 1자리 단위로 반올림 처리하여 출력하세요.
SELECT 
ename, sal, sal*1.15 기본,
round(sal*1.15,-2) 인상금액1,
round(sal*1.15,1) 인상금액2
FROM emp;
...
-- ex) 사번이 짝수인 데이터를 사번, 이름, 구분자(0/1)로 표시하세요
SELECT *FROM emp;
...
-- 사번을 기준으로 3의 배수인 경우만 출력하세요..[개인과제]
SELECT empno, ename,
MOD(empno,3) "구분"
FROM emp
WHERE MOD(empno,3)=0;
