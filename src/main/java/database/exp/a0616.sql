...
--	직책	최고급여
	...
	GROUP BY job 
	HAVING max(sal)>=2000;
-- 6. 조인테이블의 연결이 없는 카타시안 조인과 equi join의 차이점을 기술하세요
...
-- 7. 급여가 2400~4000사이의 사원의 번호, 이름, 부서명을 출력하세요.
	SELECT empno, ename, dname
	FROM emp e, dept d 
	WHERE e.deptno = d.deptno 
	AND sal BETWEEN 2400 AND 4000;

-- 8. non equi join이란 무엇인가?
--	두개의 테이블의 데이터를 비교하여 = (비교연산자)가 아닌 범위나 다른
--	형태로 데이터를 비교하여 join 하는 것을 말한다.
--	ex) 급여 동급 테이블과 사원테이블의 급여를 between로 비교하는 것이
--		예시로 들 수 있다.
