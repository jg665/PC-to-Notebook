...
-- ex1) 급여가 1000~3000사이의 부서명, 사원명, 급여를 출력.
SELECT dname, ename, sal 
FROM emp e, dept d 
WHERE e.deptno = d.deptno
AND sal BETWEEN 1000 AND 3000;
-- ex2) 부서위치(loc)별 평균급여를 출력. 부서위치 평균급여
SELECT loc, avg(sal)
FROM emp e, dept d 
WHERE e.deptno = d.deptno 
GROUP BY loc;
...
SELECT max(hiredate) "최근", min(hiredate) "최초" FROM emp;

SELECT deptno, max(hiredate) "부서별최근입사일" FROM emp 
GROUP BY deptno;

SELECT * 
FROM emp e, dept d 
WHERE e.deptno = d.deptno;
-- 두개의 테이블을 조인 관계 안에서 group 처리를 하였다.
SELECT dname, max(hiredate)
FROM emp e, dept d 
WHERE e.deptno = d.deptno
GROUP BY dname;
-- ex) 부서위치별(loc), 최초입사한사람의입사일 출력
...
-- ex2) 전체 평균연봉을 확인하고, 평균연봉이상은 상급, 미만은 하급으로 나누어,
--		부서명 사원명 연봉 연봉구분을 부서번호가 10,20인 경우를 출력하세요.
SELECT avg(sal) FROM emp; -- 평균 연봉
/*
...
--		팀(청/백) : 사원번호 홀수일때 청, 짝수일때 백팀으로 처리
-- mod(empno,2), decode() : 중첩함수처리
SELECT dname, mod(empno,2) div,  ename, 
decode(MOD(empno,2),0,'백','청')팀
from emp e, dept d
...
-- dept테이블의 deptno를 연동해서 두 테이블의 정보를 다 확인하기 위하여 사용한다.
SELECT ename, dname,sal -- 컬럼
FROM emp e, dept d  
WHERE e.deptno = d.deptno
AND sal>2000; 
-- 전체가 연결을 만들어 하나의 테이블로 사용할 수 있다.
-- 1. select로 선택해서 보여주고자 하는 컬럼을 출력할 수 있고,
-- 2. where 조건을 통해서 조회 조건을 만들수 있다.
-- 3. 여러 함수나 group함수를 활용할 수 있다.
