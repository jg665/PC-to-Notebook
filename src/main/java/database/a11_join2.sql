...
WHERE sal BETWEEN losal AND hisal
AND grade = 4;
--- ex2) SMITH의 급여와 등급을 추력하세요.
SELECT ename ,sal, grade
FROM emp e, SALGRADE s
WHERE sal BETWEEN losal AND hisal 
AND ename ='SMITH';
--- ex3) job이 SALESAMN의 급여와 등급을 출력하세요.
...
NULL 에 대해서 어떠한 연산을 적용하더라도 연산 결과는 NULL
	일반적인 EQUI JOIN 의 예 : 
	학생 에티블의 학과번호 칼럼과 부서 테이블의 부서번호 칼럼에 대한
...
-- 도 join 현황에서 확인하고자 할 때, outjoin을 활용한다.

--ex1) 사원테이블과 부서테이블을 아웃조인 하여, 아래와 같이 출력,
-- 		사원명 부서명 : 사원명이 할당되지 않았을 때, nvl을 이용해 할당된 사원 없음 표시.
SELECT nvl(ename, '사원없음'), dname
FROM emp e, dept d
WHERE e.deptno(+) = d.deptno;
--ex2) 부서별 인원수를 표시하되 부서에 할당된 인원이 없으면 0으로 표기
-- 	부서명 인원수 	2조  hint ) count(해당컬럼명시)
-- count(e.deptno) : 
