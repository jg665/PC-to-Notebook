...
4. 뷰의 한계
	1) 제한적 사용: 무결성 제약조건, 표현식,
		group by 유무에 따라 DML명령문은 제한적으로 사용된다.
...
-- 미치거나 받는다.
-- ex) 부서별 인원, 최대급여, 최소급여울 v_emp_sum으로 만들어 조회 
...
2. sqlplus
3. system/1111 로 접속
4. grant connect,resource,unlimited tablespace to scott;
명령문 수행.
5. debeaver 왼쪽 접속된 내용에서 검증/재연결 처리.
6. view 생성 확인
*/ 
/*
# 인라인뷰(inline view)
...
-- view에 DML 처리
INSERT view_emp31 values(7999,'ANGEL',7000,10);
...
-- 복합 뷰의 데이터 입력
INSERT INTO view_emp_ values(8000, 'HIMAN', 3000, 20, '인사','서울');
-- cannot modify a column which maps to a non key-preserved table
-- 통합뷰는 에러가 가능하다
-- 뷰의 삭제
-- drop view 뷰이름
DROP VIEW VIEW_emp_detp;
-- ex) 복합뷰연습.
...
CREATE VIEW view_emp_dept 
AS SELECT e.*,s.slagrade, e.deptno,d.deptno, d.dname, d.loc
FROM emp31 e, salgrade s, dept31 d 
