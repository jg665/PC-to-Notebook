...
3.	단일행 함수
	1) 데이터 값을 조직하는데 주로 사용한다.
	2)	행별로 함수를 적용하여 하나의 결과를 반환하는 함수이다.
	3)	단일행 함수의 종류
			문자함수(upper, lower, initcap),
			숫자함수(ceil, trunc, round), 
			날짜함수(add_months),
			변환함수(묵시적데이터변환/명시적데이터변환- 문자==>숫자,
				숫자==>날짜, 날짜 ==> 문자),
			일반함수 nvl, decode, upper, initcap
...
2.	substr(문자열데이터, 시작위치, 시작위치에서 갯수) : 
	...
	3x) substr('sql*plus' ,5,4) ==> plus 
		sql*plus 		 //5부터 시작해서 4개
		12345		p부터 추출
			plus
			1234	4개부터 추출	
 	...
 	ex) 코드성 데이터 - 사번, 주민번호, 학번에는 각각의 위치별로 의미하는
 		바가 있기 때문에 추출하여
 		데이터가 활용한다.	주민번호의 경우 생년월일을 위치에 따라 추출할 수 있다.
...
-- ex1) || 쓰지말고 concat를 이중으로 활용하여, 사원명-사번 을 형식으로 표기하세요
	SELECT ename, empno, concat(ename,-empno)
	FROM emp;
...
--		substr을 활용하여 입사일을 @@@@년 @@월 @@일로 출력하세요.[3]
--		''||hiredate를 통해서 문자열 형태로 출력되는 것을 확인
--		YYYY 형식에 맞게 '19'를 || 통해서 연결해준다.
--		그 중에서 연도만 추출하여 substr(데이터, 1, 4)
--		해당 데이터를 0 index부터 시작하여 4개만 추출 처리.(substr)
...
WHERE substr(hiredate,1,2)='80';
	...
	SELECT empno, ename, hiredate,
	substr('19'||hiredate ,0,3)||substr(''||hiredate ,2,4)
	||substr(''||hiredate ,6,8) "@@@@년 @@월 @@일"
	FROM emp;
