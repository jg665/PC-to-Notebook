...
public class A03_OnLineBank {
	...
	public static void main(String[] args) {
		...
		u3.start();
		u4.start();
		u5.start(); // 여러명이 함께 계좌업무 보는 것 확인 가능!
		
// ex1) GameUser로 쓰레드를 생성(사용자 이름)하고, 
// 기본 예제와 같이 게임사이트 접속, 대화하기, 게임실행하기, 게임로그아웃 배열로 쓰레드를 5개 만들어 처리하세요.
		GameUser g1 = new GameUser("김김김");
		GameUser g2 = new GameUser("홍홍홍");
		GameUser g3 = new GameUser("박박박");
		GameUser g4 = new GameUser("양양양");
		GameUser g5 = new GameUser("신신신");
		System.out.println("# 게임유저 쓰레드 시작 #");
		g1.start();
		g2.start();
		g3.start();
		g4.start();
		g5.start();
		// for문을 통해서도 간단하게 처리 가능!
...
class GameUser extends Thread{
	...
	public void run() {
		String[] gameproc = {"게임사이트 접속","대화하기","게임실행하기","게임로그아웃"};
		for(int idx=0;idx<gameproc.length;idx++) {
			System.out.println("["+idx+1+"]"+name+" "+gameproc[idx]);
		}
	}
}
