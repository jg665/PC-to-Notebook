...
public class A01_0504 {
	...
	public static void main(String[] args) {
		...
		//3. 변수와 사칙연산자와 나머지 연산을 활용하여 아래와 같은 내용을 처리하세요
		...
		//	2) 20개의 빵을 3마리의 곰돌이가 동일하게 먹은 갯수와 남은 나머지
			...
			System.out.println("20개의 빵을 3마리의 곰돌이가 동일하게 먹은 갯수: "+(totBread/bearCnt)+
					"와 남은 나머지: "+(totBread%bearCnt));
		
		//	3) 총 잔액 100000에서 교통비 3000 2회, 식대 7000, 간식비 3000을
		//	  뺀 나머지
		
		int money = 100000;
		System.out.println("총 잔액 100000에서 교통비 3000 2회, 식대 7000, 간식비 3000을 뺀 나머지: "
							+(money-(3000*2+7000+3000)));
		
		//4. 컴퓨터 가격이 1000000이상이면 고가 그 이외는 저가로 처리하기로 했다.
		//   해당 프로그램을 조건문에 의하여 처리하고 출력하세요
		
		int price = 1000000;
		// ()를 통해서 문자열 구분 : 괄호 최우선, 곱셈/나눗셈, 덧셈/뻴셈
		if(price>=100000) {
		...
		//5. 반복문을 이용하여 40~50까지 출력하고, 10부터 discount 0까지 출력하세요

		for(int cnt=40;cnt<=50;cnt++) {
			System.out.println("카운트1: "+cnt+"/t");
		}
		
		for(int cnt=10;cnt>=0;cnt--) {
			System.out.println("카운트2: "+cnt+"/t");
		}
