...
# 객체의 범위
1. 사용자 정의 클래스의 객체
2. primitive data유형의 배열도 객체에 포함된다.

# 배열의 선언
1. 데이터유형[] 참조명 = new 데이터유형[배열의 크기];
	배열의 메모리만 선언
	ex) int [] arry01 = new int[5]; : 정수형 데이터 5개를 만들다, 껍데기만 만들어둠
		arry01[0] arry01[1] arry01[2] arry01[3] arry01[4] 가 만들어진 상태
		= 데이터 할당이 안되어있지만 heap 영역에 객체의 메모리가 만들어져있다.
2. 데이터유형[] 참조명 = {데이터1, 데이터2, 데이터3};
	배열의 메모리 선언과 함께 데이터 할당
	ex) String [] fruits = {"사과", "바나나", "오렌지"}
	
# 배열의 데이터 확인
1. 배열명[index]로 데이터를 확인할 수 있다.
	index는 0부터 시작
2. 배열명.length를 통해서 배열의 크기(갯수)를 확인할 수 있다.
...
*/
	...
	public static void main(String[] args) {
		...
		String [] arry03 = {"어린이날","크리스마스","석가탄신일"};
		...
		// ex2) 주로 타는 버스번호(정수) 3개를 선언하고 할당, 출력
		int [] busnos = {13, 401, 4312};
