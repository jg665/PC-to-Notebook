...
/*
ex) 코드 계층적 block 처리
아래의 Calcu 클래스를 선언하고 입력값 2개로 4칙연산을 처리하는 코드를 처리하되
변수의 중괄호 선언 규칙에 의해서 선언하고 할당,
main()을 통해서 호출하세요
*/
class Calcu{
	// 필드 선언
	// 1. 이 클래스 블록({}) 안에서 다 사용할 수 있는 전역변수
	// 2. 이 클래스에서 모든 구성 멤버가 사용할 수 있다.
	private int num01;
	private int num02;
	// 생성자 선언
	...
	int plus() {
//		return num03+nunm04; 지역변수이기에 다른 중괄호 블럭에서 사용 불가
