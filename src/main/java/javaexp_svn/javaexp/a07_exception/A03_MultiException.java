...
1. 일반(컴파일 체크) 예외
	...
	ex) stream 관련 예외, 메모리 로딩 관련,
		DB처리 등 io가 발생하는 경우 반드시 예외를 처리하여야
		컴파일이 가능하다. (필수 예외 처리)
				*/
		/*
...
2. 실행 예외(RuntimeException)
	...
	3) 여러가지 실행 예외의 종류
		NullPointerException : 객체 참조가 없는 상태(객체가 생성되지 않음)에서
							   객체의 멤버를 호출하는 경우 발생
		...
		System.out.println(args[0]);
			...
			System.out.println("# arg의 해당 index 값이 없음 #");
...
3. 여러가지 예외 처리
	...
	3) 예외를 발생하든 발생하지 않든 처리할 구문은 finally{}에 처리한다.
				*/
			...
			} catch(Exception e) { // 예외클래스의 최상위 클래스
				System.out.println("기타 예외 발생: "+e.getMessage());
			} finally {
				System.out.println("예외 상관없이 수행할 구문 처리");
			}
...
//		String[] arry2 = {"사과","바나나"};
//			arry2[2]
//		위 코드(2개예외)와 기타 예외(최상위 예외)를 처리하고, 예외 발생 상관없이 처리할 block를 처리하세요.
			...
			try {
				...
				System.out.println("발생 상관없이 처리할 block");
			}
