...
public class A01_ExceptionBegin {
	...
	public static void main(String[] args) {
		...
		/*
# 예외 처리
1. 자바는 프로그래밍의 안정성 확보를 위해 예외 처리를 지원하고 있다.
2. 예외 처리를 통해서 예외가 나더라도 프로그램 종료가 가능하도록 한다.
3. 기본 형식
	try {
		예외가 나타날 가망성이 있는 코드
	}catch(Exception e){
...
4. 예외 처리의 계층 구조화 및 finally 블럭
	1) 먼저 최하위의 예외 클래스를 수행
	2) 최종적으로 예외의 상위 클래스인 Exception 을 수행
	3) 예외가 발생하는 순간 예외는 해당 시점(라인)에서 바로 예외를 던진다
	   예외에 해당하는 block으로 catch로 예외를 잡게 된다.
	4) finally 구문은 예외가 발생했던 발생하지 않았던 수행할 필요성이 있는 코드를 선언하여 처리
	
	try {
	
		1
		2
		3 예외 발생 => 예외 throw / 그 다음에 있는 라인은 수행하지 않는다
		4
		5
	...
	7) 사용자 정의 예외 : 
	   class 사용자정의예외 extends Exception {} 형식으로 선언할 수 있다.
		...
		System.out.println(1/0); // 오류 발생 지점
		...
		// 		String s=null; s.toString(); 코드가 예외 발생하더라도 수행될 수 있게 처리하세요
