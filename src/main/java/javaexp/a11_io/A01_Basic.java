...
3. 입력 처리를 하는 InputStream!!!
	데이터 처리 -- 정수형 ==> char ==> 문자열 ==> 객체(파일)
 	...
 	1) 주요메서드
 		...
 		이것을char로 코드값과 연결되어 있는 문자로 표현하여
 		처리한다.
 * 
 */
		...
		char c01 = 'A'; //car는 ''로 한자만 입력할 수 있다.
		// char가 모여서 된 배열이 String(문자열)타입으로 객체 만들어짐
		...
		// 자바에서는 필수 예외처리를 하고 있다.
		// 이 때 반드시 예외처리를 해 줘야지 컴파일이 가능하다.
		//		try num2 = is.read(); {
		try {
		...
		//에러가능성이 많은 부분은 필수 예외처리 한다.
		//declaration 메서드에서 위임하라
		// trycatch로 처리하겠다.
