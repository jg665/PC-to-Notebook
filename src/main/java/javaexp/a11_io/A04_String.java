...
확장된 기능을 만들어서 가는 패턴으로 구성되어 있다.
	InputStream : char ==> char[] 
		1byte의 문자만 처리(한글 특수 입력 불가능)
		
	int ==> char ==> char[] //c프로그램까지  ==> String //자바에서 생김
								 			(객체지향프로그램)
...
4. InputStream의 하위 객체들..
	1) InputStreamReader : 2byte이상 문자열을 처리해준다.(한글입력가능)
	2) BufferedReader : 객체에서는 입력된 내용을 효율적으로 사용하기 위하여
		Buffer메모리를 사용하는데, readLine() 메서드를 통해서
		문자를 하나하나 다 받아서 문자열을 만들어 enter키를 치는 순간
		 단위 데이터 입력 받아서 처리해준다.
		 */
		// System.in : 자바에서 지원하는 입력 기본 Stream 객체
		//	Console창에서 입력되는 내용을 InputStream객체에 할당한다.
//		InputStream is = System.in; //객체가 만들어졌기에 선언됨.
		BufferedReader buffer = //BufferedReader객체생성
				new BufferedReader(
						new InputStreamReader(System.in)); //제일 말단 내용
