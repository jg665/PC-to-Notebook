...
# 입력스트림과 출력스트림
0. 
1. 프로그램을 기준으로 특정한 입력도구나 프로그램을 통해서
해당 데이터를 입력받게 해주는 객체가 입력스트림이다.
		1) 출발지 : 키보드, 파일, 프로그램
		2) 도착지 : 입력받은 데이터를 처리
		3) 입력스트림 : 출발지와 도착지 사이에 데이터를 전달해주는
			객체를 말한다.
2.	프로그램을 기준으로 데이터를 대상 출력 도구나 프로그램에
출력하게 해주는 객체가 출력스트림이다.
	1) 출발지 : 프로그램을 통한 명령
	2) 도착지 : 모니터, 저장파일, 프로그램
	3) 출력스트림 : 출발지와 도착지 사이에 데이터 전달해주는
		객체.
3. 바이트 기반 스트림과 문자기반 스트림
	1) 바이트 기반 스트림
...
4.	InputStream
	...
	2) 상속 관계
		...
		- DataInputStream
	3)	주요 메서드 
		read() : byte 으로 입력한 문자 1자를 읽기 처리.
			(char)캐스팅을 통해서 처리
		read(byte[] b) : 입력한 문자열을 byte[]에 담아서 처리
		read(byte[] b, int off, int len)
	 	 : 입력한 문자열 byte[] 중에서 특정한 위치에 있는 부분을
	 	 	추출처리.
	 	 close() : Stream을 통해 처리한 시스템 자원을 반납하고
	 	 	입력 스트림을 닫는다.
	 	 	
	 	 // 1. 입력스트림 객체 System.in(자바의 기본 입력 처리)
				통해서 객체 생성.
 */	
		InputStream is = System.in;
		// 2. 입력받은 문자열의 크기 지정
		byte[] brr = new byte[5]; // 문자열 3개 입력받는 배열
			
		// 3. read(입력받아서 저장 배열 선언) 
		try {
			System.out.print("문자를 입력하세요:");
			// 입력받은 문자열을 byte[] b 에 할당 처리한다.
			int n = is.read(brr);
