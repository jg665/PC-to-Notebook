...
public class Person {
	...
	public Person(String name, int age, String loc) { //선언된 형식에 문자,갯수,맞아야 한다.
		
		// 입력 매개변수와 this.필드의 속성으로 구분하여 데이터를
		// 초기화(초기값 할당)을 처리한다.
		// 매개변수로 들어오는 데이터는 생성자에서만 사용할 수 있는 변수
		// 즉, 지역변수이다.
		// 이것을 = (대입연산)와 this.필드명으로 구분된 전역변수
		this.name = name;
		this.age = age;
		this.loc = loc;
	...
	리턴타입 : 리턴할 데이터의 type 맞게 처리한다.
		만일 리턴할 데이터가 없으면 void(비워있다)로 선언할 수 있다.
	
	#main()에서 호출..
	Person p01 = new Person(@@@, @@ ); 생성자는 한번만 호출
	메서드는 여러번 호출 가능 : 객체명.메서드명();
	p01.show();
	p01.show();
	p01.show();
