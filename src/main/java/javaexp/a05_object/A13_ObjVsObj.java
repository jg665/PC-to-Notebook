 ...
 */
class Woman{ // 우먼 클래스 객체는 다 됨.
	private String name; //속성값
	private int age;
...
class Man{
	...
	private Woman woman; // 여러가지 속성 (Woman)
...
//클래스 선언, 앞으로 객체로 만들어질 준비 중
class Person03{
	private Car ownerCar; // 한번에 여러가지 속성 포함 객체, 객체 메모리 외부로 할당 준비
...
class Mart{
	...
	public Mart(String name) {
		super();
		System.out.println("# 어서오세요"+ name+ "마트입니다.!!##");
	...
	public void buy(Product04 p) {
		list.add(p);
		System.out.println(p.getName()+"를 "+
		p.getCnt()+"개를 카트에 담았습니다");
	}
	public int calcu() {
