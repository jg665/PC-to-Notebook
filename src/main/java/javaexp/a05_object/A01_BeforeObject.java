...
//							new 객체();로 선언 없이 사용할 멤버
//						클래스명.필드; 클래스명.메서드()
	//  리턴할type	  메소드명(들어올 데이터 유형 선언-매개변수 parameter type)
	//				   plus(30,20) 50이라는 데이터 존재
    //				   num01 = 30    num02 = 20
	
	static int plus(int num01, int num02) { //특정한 생성(new메서드 없이)없이 
		// 함수를 통해서 처리할 프로세스
		int tot = num01+num02;
		// 리턴할 실제 데이터
		// 함수를 호출한 곳에 데이터를 전달
	...
	// ex1) minus 두개의 값을 뺄셈 처리 한 데이터를 리턴
	...
	static String login(String id, String pass) {
		String result=""; 
		// 비교연산자1 &&(논리연산) 비교연산자2 : himan이고 777일 때만 로그인
		// 성공 그외에는 로그인 실패 처리..
...
# 함수
	자바에서는 기능함수를 클래스 소속 기능 구성요소인 메소드 개념으로 활용한다.
	ex) 자판기 기능..
	기본 형식
