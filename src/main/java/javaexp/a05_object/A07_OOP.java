...
2.	객체(Object)란?
	...
	2) 추상적인 것(회사, 날짜) 중에서 자신의 속성과 동적을 가지고
		프로그램적으로 의미가 있는 것.
...
2.	생성자
	...
	2)	default 생성자 :
		...
		동일한 매개변수가 없는 생성자가 내부적으로 
		호출되어 사용할 수 있다.
	3)	사용자 생성자를 선언하는 순간, default 생성자는 
		사라지게 된다.
	4)	생성자는 this(매개변수데이터)로 생성자를 호출할 수 있다.
	
3.	메서드.
	1) 객체 생성후, 기능적인 처리를 하는 함수를 말한다.
	2) 구성요소로 리턴할 타입, 메서드명, 매개변수, 실행처리할 내용 등이 
		있다.
	3) 리턴 타입은 해당 기능 메서드를 호출했을 호출한 곳에 전달할 데이터
		유형을 선언하고, 실제 데이터를 전달 하는것을 말한다.
	...
	5)	메서드는 입력값과 리턴사이에 특정한 프로세스를 처리한다.
		반복적이고 재활용해야한 내용을 기능 메서드를 통해서 처리할 수 
		있다.
...
// ex) Z03_Computer 클래스를 선언하고, 속성으로 제조사/cpu/ram의 속성을 
		...
		c1.cpu="intel i5";
		c1.ram="16gb";
		System.out.println("제조사:"+c1.brand);
		...
		c2.cpu="intel i3";
		c2.ram="8gb";
		System.out.println("제조사:"+c2.brand);
		System.out.println("CPU:"+c2.cpu);
		System.out.println("RAM:"+c2.ram);
		
		
		
//		Person p1 = new Person(); // default 생성자 가능
//		매개변수로 문자열을 입력하는 생성자를 선언하는 순간
//		default 생성자는 사라지게 된다.
//		생성자는 여러가지를 overloading 규칙에 의해 생성할 수 있다. overloading = 이름이 같더라도 매개변수의 타입 갯수 순서에 따라 처리될수 있다는게 오버로딩
...
//		선언-가격할당/매개변수 없는 생성자 선언/매개변수2개 있는 생성자 선언-가격,갯수
//		로 할당하여 처리결과를 출력
		
		
		
		Note n1 = new Note(); // default 생성자 호출
		System.out.println(n1.price+":"+n1.cnt);
		Note n2 = new Note(3000); 
		System.out.println(n2.price+":"+n2.cnt);
		Note n3 = new Note(3000,5);
...
class Member{
	...
	public Member(String id, String pass, String name) {
		this(id,pass);// 선언된 생성자를 활용하여 데이터를 처리를 효과적으로 할 수있다.
		this.name = name;
	}
...
class Person{
	...
	Person(String s){
		this(); // 현재 가지고 있는 이 클래스에서 선언한 다른 생성자를 호출할 수 있다.
		name = s;
		
		System.out.println("문자열 매개변수가 있는 생성자1");
	 //매개변수로 문자열 입력하는 생성자 생성시 default 생성자는 사라진다.
	}
}
...
class Note{
	...
	Note(int p){ // 가격 할당
		price = p;
		System.out.println("매개변수1개 있는 생성자 선언-가격할당");
	}
