...
1. 데이터유형[] 참조명 = new 데이터유형[배열의 크기];
	배열의 메모리만 선언
	ex) int [] arry01 = new int[5]; 정수형 5개들이 배열을 만든다. 빈 껍데기 생성
		arry01[0] arry01[1] arry01[2] arry01[3] arry01[4]
		데이터는 할당이 안됨. heap영역에 객체가 메모리를 만들어져 있다.
2. 데이터유형[] 참조명 = {데이터1, 데이터2, 데이터3};
	배열의 메모리선언과 함께 데이터 할당.
	ex) String []friuts = {"사과","바나나","오렌지"}
# 배열의 데이터 확인
