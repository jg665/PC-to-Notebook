...
/*
2. null이란
	1) 객체를 선언만하고, heap영역에 생성이 되지 않을 때, null 즉 heqp역역에
		아무것이 없다는 뜻으로 사용되는 것을 말한다.
	2) heap영역에 없는 객체를 속성/메서드를 호출하면 NullPointerException 발생한다.
	3) 일반적으로 해당 데이터가 생성 되었을 때, 처리하기 위하여
		객체 변수는 if(변수명!=null)로 check 후, 속성을 사용한다.
3. 비교 연산자 처리..
 	1) 비교 연산자(==,!=...)는 stack영역의 할당된 데이터를 비교하며,
 		boolean값을 리턴한다.
 	2) 기본 primitive 데이터 : 할당된 데이터값 비교.
 	3) 객체형 참조변수 : heap영역의 주소값 비교.
 		참조변수!=null : 객체가 할당되는 경우를 의미
 	...
 	4) String 
		...
		일반적으로 외부 입력 Scanner이나 Stream(네트웍,파일처리)을 통해서 
 ...
 */
			...
			System.out.println(p01.age);
				...
				System.out.println("객체 생성되지 않았습니다.");
