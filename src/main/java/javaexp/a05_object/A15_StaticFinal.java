 ...
 */
		...
		p2.getAge();
		p1.age=25;
//		p1.name="마길동"; final: 객체 단위로 한번 할당된 것은 변경 불가.
		
//		Person04.KIND="동물"; 클래스 단위로 한번 할당된 것은 변경 불가.
		
//	ex) Animal final만 붙은 상수로 동물의 종류, static final로 동물로 설정.
//				일반변수로 먹이의 종류 *
//			eatFood() 먹이를 먹다.
//			eatFood()
//			(동물)인 [사자]가 @@@을 먹다.
...
class Person04{
	int age;
	final String name; // 객체단위로 변경 불가
	static int EARTH_RESOURCE; // 객체 공유 메모리
	static final String KIND="사람"; // 클래스로 한번 할당 변경 불가/ 생성자로도 할당 안됨
	public Person04(String name) {
		this.name = name;
	}
	public void getAge() {
		age++;
		System.out.println(KIND+"인 "+name+"의 나이를 먹다"+age);
	}
