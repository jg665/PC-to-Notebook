...
//				ex) 참조변수명, 메서드명 소문자로 시작
			int fruitCnt = 15; //(int를 가독성으로 구분해서 과일의 갯수로 표시)
			String studentName="홍길동";
			Vector v01; //(객체의 참조변수);
//			void setName(), void callFriend()
//				cf) 클래스명, 생성자명은 관례로 대문자로 시작한다.
//					class Person{ Person(생성자명)(){}} // 자바 클래스명은 대문자로 만든다.
//			5) 문자 수 길이 제한이 없다.
//				너무 간단하게 해서 본인이 이 변수를 나중에 이해하지 못 할 정도로 
//				하지 말아야 된다. i, j, k... n
//			idx(인덱스 변수명) , setName(이름을 저장하는 변수명)
//			6) 자바 예악어는(명령어로 내장된 것)은 변수명으로 사용할 수 없다.
//			String break;(X)
//			int if;(X)
			String break02;
			String break01; 
...
//			4) 선언과 저장
			String name = "홍길동";
			// 지역변수(메서드 안에 선언된 변수)는 반드시 초기화를 호출하여야
			// 컴파일이 가능 하다.
			...
			/*
			초기화 : 데이터 할당
			int num01; //데이터 할당을 안하고 바로 쓰면 에러가 난다.
			System.out.println(num01); (X)*/
			int num04=25; // 지역변수의 초기화 처리를 하여서 호출해서 사용할 수 있다.
