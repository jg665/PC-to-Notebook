...
1. 기본적으로 데이터의 처리방식이 
	시간 ==> 일==> 주 ==> 월==> 년
	단위로 처리되는 객체를 생성하고 활용할 수 있게
	처리해준다.
	내부적으로 메커니즘이 10진수와 다르다.
...
*/
		...
		//		클래스명.대문자 : fianl이라는 필드에 붙으면 상수(변경되지 않는 수)
		...
		// 		= 년도의 상수값. Calendar.상수 : 객체생성없이 사용하는 static이고
		//			상수인 대문자를 가지고 있다.
		...
		// 대한민국 월 문화와 미국의 문화는 차이가 있다. 미국에선 1월,2월,3월 이런식으로 쓰지 않는다.
		// 월은 영미권에서 고유의 문자(jan,feb,mar...)로 되어 있어서 우리나라와 체계가 차이가 있다.
		// 0부터 11까지 index로 매핑되어 있다. +1로 우리나라의 1월~12월과 연결 처리.
 ...
 */
		...
		// 월(month)은 0~11의 index범위로 설정하여 저장한다. ex)10월 = 9
		cal2.set(2021, 9, 5); //2021.10.5
		// cal2.get("년도"); 이럴경우 heap영역 메모리 사용이 커서 X, 문자열 통해 조건을 처리하면 메모리 비효율(X)
...
/*
	ex) cal3를 만들고 자신의 생년월일의 설정하고, 다시 출력하세요.
		 
		 */
		Calendar cal3 = Calendar.getInstance();
		cal3.set(1994, 11, 2);
		//cal3.set(매개변수의 갯수와 유형) : 매개변수의 갯수와 유형에 따라서
		// 여러가지 데이터를 저장할 수 있다.
