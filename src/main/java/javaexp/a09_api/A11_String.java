...
1. 숫자
	...
	{'H','e','l','l','o'}
	==>
	{25,34,72,43,22} : 코드값으로 bytes 배열은 문자가 모인 
	문자열로 사용할 수 있다.
	==> 객체지향 프로그램에서 char[] 배열에서 한단계 더 나아가
	String이라는 문자열 객체를 만들게 되었다. c++부터 사용되기 시작하였다.
 ...
 */
		//유니코드 값을 byte로 받아서 맵핑되는 문자들을 호출해서 가져옴.
		...
		System.out.println((int)'!');
// ex) 위와같은 byte 배열과 유니코드값을 이용해서 Good job을 출력하세요[6]
		byte[] bytes1 = {71,79,79,68,74,79,66};
...
//ex) 오늘도 즐겁고 감사한 하루!! encoding으로 euc-kr과 utf-8로 
//		처리하여 출력하세요 [2조]
		...
		try {
			...
			System.out.println("문자열 B:"+strB);
			
			byte[] byte2 = str3.getBytes("UTF-8");
			String strC = new String(byte2);
			System.out.println("문자열 C:"+strC);
...
/*
 ex) 메뉴는 사과, 바나나, 딸기 입니다.
	...
	*/
			...
			case 13 : System.out.println("세번째 과일 사과를 선택했습니다.."); break;
			}
			
	}
}
// 71 79 79 68 74 79 66 good job
