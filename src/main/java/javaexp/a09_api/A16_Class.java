...
/*
# Class
1. 클래스와 인터페이스 메터 데이터 관리
	메터데이터 : 클래스의 이름, 생성자, 필드, 메서드 정보
2. Class의 객체 얻기
	1) getClass() ex) Class clazz = obj.getClass();
 ...
 */
		...
		// clazz1를 통해서 Car 클래스의 메타정보를 가져온다.
		...
		로 Class 객체 메타정보를 위한 생성 2가지 방식으로 처리하고,
			객체 Food를 Class에 의해 생성하고, 기능메서드를 활용하세요.[2]
		*/
			Food f1 = new Food();
			Class clazz3 = c1.getClass();
			System.out.println(clazz3.getName());
			System.out.println(clazz3.getSimpleName());
			System.out.println(clazz3.getPackage().getName());
			try {
				Class clazz4 = Class.forName("javaexp.z01_vo.Food");
			...
			try {
				Food f2 = (Food) clazz3.newInstance();
				f2.setName("짜장면");
