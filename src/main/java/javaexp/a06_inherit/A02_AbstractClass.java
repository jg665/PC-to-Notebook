...
//		3. 다형성(상위클래스를 참조로 하위 클래스 생성이 가능하다)
	...
	//		추상메서드 working()
	//		PoliceMan 상속받은 실제 클래스 working() 치안을 유지하다. (오버라이딩)
	//		FireMan 상속받은 실제 클래스 working() 불을 끄다. (오버라이딩)
...
abstract class Worker{
	...
	abstract void working(); //상속받은 내용들 오버라이딩 해서 쓴다.
}
...
// 3. 추상클래스를 상속받은 하위 클래스는 추상메서드를 반드시
// 재정의하여 사용하여야 한다.
