...
# 메소드 재정의 (override)
1.	부모 클래스의 상속 메소드 수정해 자식 클래스에 재정의 하는것을 말한다.
2.	메소드 재정의 조건
		1) 부모 클래스의 메소드와 동일한 이름으로 사용하여야 한다.
		2)	접근 제한을 더 강하게 재정의는 불가하다.
			- 상위 메서드가 public이면 default나 private으로 재정의 불가능
...
3.	메서드 : 자식클래스에서 재정의 불가


오버로딩  : 기존 메소드에 매개변수 추가, 상속이랑 관계 없다.
오버라이딩 : 부모클래스와 자식 클래스에 동일한 메서드 사용 가능
...
//		상속받은 Tiger 클래스 사냥하다() 추가하여 처리
//	1)	생성자를 통해서 kind 설정.. eat() 재정의하여 고기를 먹다 추가.
class Animal{
	String kind;
	Animal(String kind){
		this.kind = kind;
...
class SpiderMan extends Person{
	...
	public void jump() {
		...
		System.out.println("건물 사이를 뛰면서 난다!!");
	}
	public void fireWeb() {
