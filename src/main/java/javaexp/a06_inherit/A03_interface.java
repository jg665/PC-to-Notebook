...
2. 기본 형식
	interface 인터페이스명{
		...
		void call();
		// 인터페이스는 기본적으로 public abstract가 포함되어 있기 때문에
		// 생략이 가능하다.
	}
	...
	cf) 일반 상속은 extends는 당일 상위 클래스만 가능하지만,
		인터페이스는 여러개의 상위 인터페이스를 상속 가능하다.
...
3. 기타 인터페이스의 필드와 속성
	...
	public static void main(String[] args) {
		...
		Mz m1 = new Mz();
		m1.flying();
		// 인터페이스로 매개변수가 있으면, 상속받은 여러가지 하위 클래스는
		// 다형성에 의해서 할당 할수 있다.
...
//			하위에 실제클래스로 DuckSound, BabySound, RobotSound를
//			상속받아 main()에서 호출하여 처리하세요
interface SoundWay{
	void sound();
}
...
// ex) AttackWay 인터페이스 선언.. attack(); 추상메서드 선언.
// 상속받는 실제 클래스 LaserBeam, RocketMissile..
// Mz에 AttackWay할당하는 부분과 attack()를 통해서 여러가지 공격방법에
// 따라 공격하게 처리..
interface AttackWay{
	void attack();
}
