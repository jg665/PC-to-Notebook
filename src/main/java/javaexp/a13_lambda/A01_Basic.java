...
/*
# 람다식 표현
1. 메서드(함수)에 대한 표현방식을 보다 간편하게 화살표형식으로
표현하는 것을 말한다.
2. 주로 메서드의 익명함수로 바로 사용할 때, 활용된다.
	...
	*/
	...
	public static void useFIMethod(FuncitonInterface1 fi) {
		fi.methodA();
	}
	...
	public static void main(String[] args) {
		...
		// 재정의 메서드 호출..
		s1.call01();
		// 상위인터페이스 = 하위 인터페이스명으로 하위객체 익명으로 선언.
		...
		useFIMethod(new FuncitonInterface1() { //오버라이딩 안되서 에러,실제와 동일하게 기능메서드 만들어야함. 
			@Override
			public void methodA() {
				// TODO Auto-generated method stub
		...
		// 람다식으로 표현1.. : {} 블럭을 통해서 여러라인 처리
		useFIMethod(() -> {
			System.out.println("람다식1으로 익명인터페이스 메서드 호출");
		});
		// 람다식으로 표현2 : 바로 한라인의 내용을 처리..
		useFIMethod(() -> 
		System.out.println("람다식2으로 익명인터페이스 메서드 호출"));
	}

}
interface Show{
	void call01(); //추상메서드 선언..
}
