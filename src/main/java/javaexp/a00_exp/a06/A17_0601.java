...
1. System. 하위에 기능메서드로 현재시스템에 대한 내용을 확인하고 처리할려고 한다.
	...
	.keySet() : 해당 내용을 set형식의 key를 객체들로 가져올 수 있다.
...
2. 시스템의 환경변수를 가져와서 해당 내용을 자바에서 확인할 때, 사용하는 객체와 메서드를 기술하세요.
	System.getenv("환경변수의 key")를 통해서 환경변수 설정값 가져온다.
	
3. String 문자열이 만들어질 때까지 과정을 bit단위부터 기술해보세요.
	bit 01 ==> byte ==> 코드값으로 char
	char[], byte[]으로 통해서 모인 배열이 객체형 타입으로 설정한 것이
	String이다.
	byte [] bytes = {64,27,30};
	char [] chars = {'H','e','l'};
	...
	==> 숫자들에 코드를 대입 'A', 'B' 문자를 설정.
	문자들이 배열로 만들어서 일상에 문자열 사용.
	{'H','e','l','l','o'}
	
	
	
4. String 생성자를 통해, 알파벳 대문자 8자의 임의의 비밀번호를 만들려고 한다. 
    byte[] 배열을 이용하여 처리하세요. 65-90 , math.random
 */
	byte[] pass = new byte[8];
	// 알파벳 대문자 코드 범위 : 65~90 Math.random*26+65
...
5. 학번을 연도(2자리)학년(2자리)반(1자리)성별(F/M)로 만들어서 처리할 때, 
   ...
   */
//	String studenNum = "21023FM";
//	String subStr = target.substring();
//	System.out.println(studenNum.charAt(0,3)+"년도 입학");
	...
	// ture이면 "여자"를 할당, 그렇지 않으면 "남자"를 할당 처리
...
//	String studentNum = "21023-FM";
//	String yy = studentNum.substring(0, 2);
//	String gr = studentNum.substring(2, 4);
//	String cl = studentNum.substring(4, 5);
//	String boy = studentNum.substring(6, 7);
//	String girl = studentNum.substring(7, 8);
//	System.out.println(yy+"년"+gr+"학년"+cl+"반"+boy+"남자");
//	System.out.println(yy+"년"+gr+"학년"+cl+"반"+girl+"여자");
	/*
...
6. 함수의 두가지 유형은 기술하세요..(DB)
              1) 단일 행 함수 :
                ...
                2) 복수 행 함수 :
                   조건에 따라 여러 행을 그룹화하여 그룹별로 결과를 하나씩 
