...
public class A12_0519 {
	...
	public static void main(String[] args) {
		...
		Calcu c1 = new Calcu(); // default 생성자 호출., 필드값 생성시 사라진다.
		// 다른 생성자를 선언하더라도 사라지지 않게 할려면
		//	다시 생성자를 선언한다.
		Calcu c2 = new Calcu(25,5); //객체 4개 , 사칙연산 생성
		...
		System.out.println(c2.num01 + " - "+ c2.num02 + " = "+(c3.num01-c3.num02));
		System.out.println(c2.num01 + " * "+ c2.num02 + " = "+(c4.num01*c4.num02));
		System.out.println(c2.num01 + " / "+ c2.num02 + " = "+(c5.num01/c5.num02));
...
2. default 생성자가 무엇인지 기술하고, Calcu
	...
	default 생성자란 class 선언 시, 생성자를 선언하지 않더라도
...
class Calcu{
	int num01;
	int num02;
	public Calcu() {} // default생성자 호출시 필드값 사라지니 다른 생성자 선언한다.
...
/*
3.	생성자/메서드 overloading 이란 무엇인가? 기본 예제와 함께 설명 하세요.
...
/*
4.	생성자 안에서 this()를 사용하는 목적을 기술하고, Prodcut 클래스 안에서
	물건명, 가격, 갯수를 생성자의 매개변수 없을 때, 1,2,3,개 있게 선언하고,
...
/*
5.	메서드의 기본형식을 선언하고, primitive 데이터유형, 배열, 객체를 리턴하는 것을
	선언하고, main()메서드에 해당 내용을 출력 처리하는 예제를 만드세요.
...
/*
7.	OnLineLibrary 클래스를 선언하고, 기능 메서드를 login(Member m), lent(Book b)
	을 처리하세요. login() 메서드로는 입력된 id, pass가 goodMan/7777일때 로그린 성공 그외는
	로그인 실패 문자열 리턴처리, lent() 메서드로는 반납기한 @@인 도서 @@@대출
 */
class OnLineLibrary{
