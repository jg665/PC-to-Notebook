...
//	    클래스 만든다는것 : 사람이 코딩으로 text에 기록 (사람이 인식 할 수 있는 언어)
//	    컴파일을 거쳐서 기계(컴퓨터가)인식할 수 있는 파일로 변환 .class
...
//			있는 한 것이 클래스다.
//		2)	사람이 프로그래밍을 할 떄, 가장 효과적인 방법을 아는 개념부터 시작
...
//			클래스명 상단에 선언한 package 패키지면;이 같은 경우에만
...
//			통일된  메서드명 하에서 상속의 하위 클래스에서 다양한 형태
//			선언할 수 있게 처리하는 것을 말한다.
//	6. 폴리모피즘이(polymorphism) 무엇인지 기술하세요.
...
//			추상클래스는 폴리모피증을 위해 만들어졌기 때문에, 객체 생성을 하지 못 한다.
...
//
//	2. 응집성이라는 개념을 무엇일까요? 한번 찾아 보세요
//
//	3. 프로그램의 안전성을 위해서는 어떤 작업들이 필요할까요?
//
