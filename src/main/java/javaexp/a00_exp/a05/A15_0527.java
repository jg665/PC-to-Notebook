...
/*		1. 클래스 선언에서 toString()의 역할을 기술하고. 
	...
	toString() : 모든 객체의 패지키명.클래스명@주소값으로 참조변수만 선언하여도 
		해당 내용에 대한 부분을 출력하게 정의해 놓았다.
	toString() : 내용을 재정의하면 해당 내용을 참조변수출력으로만 확인할 수 있다.
	super.toString()을 호출하면, Object에서 선언한 내용인 패지키명.클래스명@주소값으로
		가져올 수 있다.
*/
//		Car c = new Car("자동차");
//		c.go();
...
//		System.out.println();
		...
		2.	상속관계에서 super키워드를 사용하는 경우를 예제와 함께 기술하세요.
			1) 상속관계 재정의 매서드가 있을 때, 상위 메서드와 현재 메서드를 구분하기 위하여
				사용한다.
				class Father{
					private String name;
					Father(String name){
		...
		4.	오라클 설치과정과 설치에 필요한 내용을 기술하세요.
			...
			2) 설치과정
				...
				admin계정인 system/111 접속하여
				현재 폴드에 있는 scott.sql 파일을
				@(run :실행)scott.sql 통해서 계정생성과 샘플 테이블 생성 데이터 입력처리
				@scott.sql
			# client툴 dbeaver 설치	
			...
			# client툴 dbeaver 설치	
				...
				2) 상단 메뉴 데이터베이스 ==> 새 데이터베이스 연결 
					프로그램 연결..
					host : ip - localhost(현재 컴퓨터에서 서버가 설치되어 있을 때)
					port - 1521
					sid(데이터베이스 계정, 식별자) - xe
					계정 - scott
					비번 - tiger
					test 연결 확인
...
abstract class Worker01{
	...
	// 2. 추상메서드는 상속받는 하위 클래스에서 반드시 재정의 하여야 한다.(컴파일에러)
...
class Programmer01 extends Worker01{
	...
	public void working() {
		// TODO Auto-generated method stub
		System.out.println(getKind()+"이 개발을 하다!!!");
	}
}
