...
public class A02_Basic {
	...
	1. 배열은 한번 크기가 설정이 되어 만들어 지면,
		배열의 크기를 동적으로 변경이 불가능하다.
		새로운 메모리로 배열을 다시 만들어야 하는 단점을
		가지고 있다.
	...
	public static void main(String[] args) {
		...
		set.add("수박");
		set.add("수박");
		System.out.println("set 구조 확인:"+set);	//중복된 데이터 제거 시 필요.
		// 중복 허용, 순서 확보
		...
		lst.set(0, "망고");
			// lst.remove(삭제할 인덱스)
		lst.remove(lst.size()-1); // 크키-1 마지막데이터
			// list.add(위치, 특정위치에 추가)
		lst.add(1, "사과");
		for (int idx=0;idx<lst.size();idx++) {
			//get()을 통해서 단위 데이터를 가져올 수 있다.
			System.out.println(idx+1+":"+lst.get(idx));
		}
		// ex) 음료 list 내용을 3번째 위치에 추가,
		//		첫번째 위치의 음료 삭제, 4번째 위치에 음료 수정
		//		하여, 전체 데이터 리스트를 출력 처리하세요..
//		List bev=new ArrayList();
		bev.add(2, "아이스아메리카노");//음료추가
		bev.remove(0);
		bev.set(3, "맥주");//음료수정
		for (int idx=0;idx<bev.size();idx++) {
			System.out.println(idx+1+":"+bev.get(idx));
		}
		// 객체 단위로 데이터 추가 변경
		...
		plist.add(new Product("수박",2000,3));
		plist.add(new Product("키위",13000,13));
...
//			p.buy();
		...
		// ex) Food 객체를 ArrayList를 객체 3개를 생성하여
		// 담고, 해당 내용을 출력 처리하세요..
		// List<Food> : 리스트 객체에 기본 데이터 유형인 Object
		// 에서 특정한 데이터 유형 Food만 구성요소로 들어 올 수 있게
		// 하여, 반복문에서 typecasting 없이 바로 객체를 사용할 수 있게
		// 한다.  // 만약에 그냥 그대로 쓸시 object 타입이라 타입 캐스팅 필요하지만 제너릭이라는 <Food> 이 객체만 list 형태의 데이터로 들어오세요
				// 라고 추가하고 따로 타입캐스팅 데이터 타입 선언할 필요 없이 사용하는것 
		List<Food> flist = new ArrayList<Food>();
		// 객체 단위로 추가.
		// Food : 제너릭 데이터 타입, 특정한 데이터만 출력
		flist.add(new Food("김밥",2000,"오전동"));
		flist.add(new Food("라면",5000,"고천동"));
		flist.add(new Food("냉면",6000,"왕곡동"));
		flist.set(0, new Food("국밥",8000,"고천동")); //내용 변경
		for(Food f:flist) {
		...
		System.out.println("Map의 name값:"+map.get("age"));
		System.out.println("Map의 name값:"+map.get("loc"));
		
		// ex)물건명, 가격, 갯수를 key로 하고 해당 값을 value로 하여
		// 할당 후, 출력 처리 하세요..
		Map<String,String> pmap = new Hashtable<String,String>();
		pmap.put("pname", "사과");
		pmap.put("price", "3000");
		pmap.put("cnt", "5");
		System.out.println(pmap.get("pname"));
		System.out.println(pmap.get("price"));
		System.out.println(pmap.get("cnt"));
	}
