...
2.	구현 클래스
	ArrayList
	Vector : 불확실성이 많은 쓰레드에서 보다 안정적으로
		데이터를 추가 삭제할 수 있다.
	LinkedList : 인접 참조를 링크해서 체인처럼 관리
		특정 인덱스에서 객체를 제거하거나 추가하게 되면 바로
		앞뒤 링크만 변경
 ...
 */
		...
		System.out.println("isEmpty():"+list.isEmpty()); //isEmpty():true
		...
		list.add(1,"오렌지");
		System.out.println(list); //[사과, 오렌지, 바나나, 딸기]
		System.out.println("세번째:"+list.get(2)); //세번째:바나나
		list.set(2, "수박"); // 객체를 바꿈
		System.out.println(list); //[사과, 오렌지, 수박, 딸기]
		System.out.println("수박이 있는지?"+list.contains("수박")); //수박이 있는지?true
		list.remove(0);
		list.remove("바나나");
		System.out.println(list); //[오렌지, 수박, 딸기]
		list.clear(); // 전체 삭제
		System.out.println("isEmpty():"+list.isEmpty());
		System.out.println("size():"+list.size());
// ex) 회원관리 ArrayList<String>
//		회원 등록 2명, 1번째에 추가, 2번째 회원변경
//		회원 탈퇴(index, 이름) 
		...
		member.add("홍길동");
		member.add("마길동");
		System.out.println("isEmpty():" + member.isEmpty());
		System.out.println("size():"+list.size());
		...
		System.out.println("size():"+list.size());
		System.out.println(member);
		// Food(String nVector<E>nt price, String loc)
